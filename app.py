...
if st.session_state.kategori:
    ...
    # 🔊 Otomatis baca soal dari folder audio, fallback ke gTTS jika file tidak ada
    from gtts import gTTS
    import tempfile

    audio_file = os.path.join("audio", "pertanyaan.mp3")
    if not os.path.exists(audio_file):
        os.makedirs("audio", exist_ok=True)
        tts = gTTS(text="Apa nama objek ini?", lang='id')
        tmpfile = os.path.join(tempfile.gettempdir(), "fallback_pertanyaan.mp3")
        tts.save(tmpfile)
    ...
    if st.button("🔍 Periksa Jawaban") and st.session_state.jawab_dulu:
        ...
        if benar:
            ...
            audio_teks = "Benar sekali!"
        else:
            st.markdown("😢 <span style='color:red; font-size:24px;'>❌ Coba lagi ya!</span>", unsafe_allow_html=True)
            audio_teks = "Coba lagi ya!"

        # 🔊 Mainkan suara sesuai hasil jawaban
        tts = gTTS(text=audio_teks, lang='id')
        tmpfile = os.path.join(tempfile.gettempdir(), "jawaban.mp3")
        tts.save(tmpfile)
        with open(tmpfile, "rb") as f:
            audio_bytes = f.read()
        st.markdown(f"""
        ...
        # Tambahkan jeda 2 detik sebelum lanjut ke penjelasan
        time.sleep(2)
    ...
    if st.session_state.show_penjelasan:
        ...
        # Tombol untuk putar penjelasan secara manual
        if st.button("🔊 Dengarkan Penjelasan"):
            tts = gTTS(text=penjelasan, lang='id')
            tmpfile = os.path.join(tempfile.gettempdir(), "penjelasan.mp3")
            tts.save(tmpfile)
            with open(tmpfile, "rb") as f:
                audio_bytes = f.read()
            st.markdown(f"""
